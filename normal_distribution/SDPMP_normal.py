...
# stage or time i             
    ...
    # the climate scenario or MU
        s = 0
        # a climate scenario
        PMUA0 = norm.pdf((MU[i][k]),(MU[i][s]),sigma0)
        while s < NA - 1:
            s = s + 1
            PMUA0 = PMUA0 + norm.pdf((MU[i][k]),(MU[i][s]),sigma0)
        for s in range(NA):
        # a climate scenario
            (PMUA[i][k][s]) = norm.pdf((MU[i][k]),(MU[i][s]),sigma0) / PMUA0
            # norm.pdf((MU[i][k]),(MU[i][kk]),sigma0) is the probability of flow (MU[i][k]) given mean and variation

PAMU = np.zeros((( n, NA, NA )))
# initialization of the conditional probability of a current climate scenario or MU (k=1:NA) given an observed MU in a previous climate scenario (j=1:NA), at time i=1:n
# (PAMU[i][j][k]) i =1:n stage or time; j =1:NA an observed MU or previous climate scenario; k =1:NA a current climate scenario or MU

# the (first) starting stage climate scenarios info.
for i in range(1):
# stage or time i        
    for j in range(NA):
    # an observed MU or previous climate scenario       
        for k in range(NA):
        # a current climate scenario or a MU
            (PAMU[i][j][k]) = (PA[k])
    ...
    # an observed MU or previous climate scenario         
        ...
        # a current climate scenario or a MU       
            ...
            while s < NA - 1:
                ...
                # calculation for the probability of a previous observed MU, PPMU
            (PAMU[i][j][k]) = (PA[k]) * (PMUA[i-1][j][k]) / PPMU
            # the conditional probability of a climate scenario given an observed MU
                
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#STEP 2 Calculation of the backward SDP value functions

EATC = np.ones(((( n, NA, NH, NHUP )))) * largenumber
# initialization of the expected annual total cost for upgrading height m=1:NHUP and existing height l=1:NH; given an observed MU in a previous climate scenario (j=1:NA), at time i=1:n 
# (EATC[i][j][l][m]) i =1:n stage or time; j =1:NA an observed MU or previous climate scenario; l =1:NH existing levee height; m =1:NHUP upgrading levee height
...
# for upgrading height m=1:NHUP and existing height l=1:NH, given an observed MU in a previous climate scenario (j=1:NA), at time i=1:n
# (VALUE[i][j][l][m]) i =1:n stage or time; j =1:NA an observed MU or previous climate scenario; l =1:NH existing levee height; m =1:NHUP upgrading levee height
...
# given an observed MU in a previous climate scenario (j=1:NA), at time i=1:n, with current existing height l=1:NH and upgrading height m=1:NHUP
# (EMINIVALUE[i][j][l][m]) i =1:n stage or time; j =1:NA an observed MU or previous climate scenario; l =1:NH existing levee height; m =1:NHUP upgrading levee height
        ...
        # existing levee height l
            ...
            # corresponding to (EXH[i+1][]) = (EXH[i][l])+(UPH[i][m])
                ...
                # expected annual total cost only exists when existing + upgrading height is no greater than maximum height
                    (EATC[i][j][l][m]) = annualtotalcost( existh, upgradh, mu ) * ( 1/((1+R)**i) ) * (PAMU[i][j][k])
                while k < NA - 1:
                    k = k + 1
                    mu = (MU[i][k])
                    if existh + upgradh <= Hmax + 1e-6:
                        (EATC[i][j][l][m]) = (EATC[i][j][l][m]) + annualtotalcost( existh, upgradh, mu ) * ( 1/((1+R)**i) ) * (PAMU[i][j][k])
...
# initialization of the accumulation for the best decisions from all later stages, at time i=1:n, for existing height l=1:NH, given an observed MU in a previous climate scenario (j=1:NA)
# (OPTVALUE[i][l][j]) i =1:n stage or time; l =1:NH existing levee height; j =1:NA an observed MU or previous climate scenario
OPTUP = np.zeros((( n, NH, NA ))) 
# initialization of minimum upgrading height m, at time i=1:n, for existing height l=1:NH, given an observed MU in a previous climate scenario (j=1:NA)
# (OPTUP[i][l][j]) i =1:n stage or time; l =1:NH existing levee height; j =1:NA an observed MU or previous climate scenario
OPTH = np.zeros((( n, NH, NA ))) 
# initialization of minimum resulted height l, at time i=1:n, for existing height l=1:NH, given an observed MU in a previous climate scenario (j=1:NA)
# (OPTH[i][l][j]) i =1:n stage or time; l =1:NH existing levee height; j =1:NA an observed MU or previous climate scenario
...
# stage or time i                          
    ...
    # existing levee height l, starting from H0 = 0
        for j in range(NA):
            # an observed MU in a previous climate scenario, none for the first stage                          
            for m in range(NHUP):
            # upgrading levee height m
